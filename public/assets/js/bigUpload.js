function bigUpload () {
	...
	this.processFiles = function() {
		...
		//set name video
		$('#name').val(this.uploadData.file.name);
	...
	this.sendFileData = function() {
		var data = 'key=' + this.uploadData.key + '&name=' + this.uploadData.file.name + '&ext=' + this.uploadData.file.name.split('.').pop();
		xhr = new XMLHttpRequest();
		...
		xhr.onreadystatechange = function() {
				if(xhr.readyState == 4) {
					...
					//set id to global var
					$('#video_id').val(response.id);
					$('.urlwatch').html('<a href="' + rootUrl + '/watch?v=' + response.code + '">' + rootUrl + '/watch?v=' + response.code +'</a>');
	...
	this.progressUpdate = function(progress) {
		...
		if(progress % 5 === 0) {
			...
			this.$(this.settings.timeRemainingField).textContent = humanizeDuration(timeLeft*100) + ' time remaining..  ';
		}
	};
